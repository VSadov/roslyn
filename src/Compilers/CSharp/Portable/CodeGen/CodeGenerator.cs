...
namespace Microsoft.CodeAnalysis.CSharp.CodeGen
{
    internal sealed partial class CodeGenerator
    {
        ...
        // - the most encompasing expression, or
        // - the containing block, if expression is ref-assigned. (NB: not sequence, since sequence can return its locals)
        ...
        private void GenerateImpl()
        {
            ...
            Debug.Assert(!(_blockTemps?.Count > 0), "leaking block temps?");
            _blockTemps?.Free();
        }
        ...
        private void PromoteExpressionTempsToBlockTemps()
        {
            if (_expressionTemps?.Count > 0)
            {
                var blockTemps = _blockTemps ?? (_blockTemps = ArrayBuilder<LocalDefinition>.GetInstance());
                blockTemps.AddRange(_expressionTemps);
        ...
        private void ReleaseBlockTemps()
        {
            if (_blockTemps?.Count > 0)
            {
                // release in reverse order to keep same temps on top of the temp stack if possible
                for (int i = _blockTemps.Count - 1; i > 0; i--)
                {
                    var temp = _blockTemps[i];
                    FreeTemp(temp);
                }
